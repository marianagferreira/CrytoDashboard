...
def plot(year1, crypto, graph1, graph2):
    ...
    if graph2 == 'Volume':
        ...
        MA100 = df['Open'].rolling(100).mean()  # creat column of 100 day MA
        fig2 = go.Scatter(y=MA100, x=df.index, name=crypto + ' Moving Average')
...
def table_to_graph(row_data):
    df_table = table_data if row_data is None else pd.DataFrame(row_data)
    final_table = px.bar(df_table)
